    ...
    <div className="absolute top-0 left-0 right-0 z-50 p-3 sm:p-4 md:p-6 lg:p-8 pointer-events-none">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3 md:gap-4 w-full">
        ...
        {(authState === 'authenticated' || authState === 'unauthenticated') && onSearchRepo && (
          <form onSubmit={handleSearchSubmit} className="flex items-center gap-2 flex-1 max-w-md mx-auto pointer-events-auto">
            <input
              type="text"
              value={searchUrl}
              onChange={(e) => setSearchUrl(e.target.value)}
              placeholder="Search GitHub repo URL..."
              className="px-2 sm:px-3 py-1.5 sm:py-2 bg-black/50 border border-white/20 rounded text-white/90 font-mono text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 transition-shadow flex-1 w-full"
        ...
        {authState === 'loading' && (
          <div className="w-6 h-6 sm:w-8 sm:h-8 border-2 border-cyan-300/50 border-t-cyan-300 rounded-full animate-spin flex-shrink-0" />
        )}
        
        {/* Login button or Avatar on right */}
